#!/usr/bin/env python3
# 2026-02-20 12:00 v0.9.0 - DBC file support (cantools): load via Details > DBC tab,
#                           live signal decoding with physical values + units,
#                           MonitorFullScreen (F6): 2-column layout with ID selection
#                           and live decoded signal table; Message-Name in main monitor
#                           when DBC loaded; F6 added to shortcuts overlay.
"""
...
def parse_cli_args() -> StartupArgs:
    ...
    parser = argparse.ArgumentParser(
        prog="can_tui.py",
        description="Waveshare CAN Bus TUI  v0.9.0",
...
class TracePanel(Container):
    ...
    def compose(self) -> ComposeResult:
        ...
        with Horizontal(classes="trace-ctrl-row"):
            ...
            yield Button("Export CSV", id="btn-trace-export", variant="default")
...
class DetailsScreen(Screen):
    ...
    def action_pop_screen(self) -> None:
        """Return to the main screen and clear the details-screen reference."""
...
class MonitorFullScreen(Screen):
    ...
    def on_data_table_row_selected(self, event: DataTable.RowSelected) -> None:
        """Toggle selection when the user presses Enter on a row in the ID table."""
        if event.data_table.id == "fs-id-table":
            self._toggle_row_by_key(str(event.row_key.value))

    def action_toggle_selected(self) -> None:
        """Space bar: toggle the currently highlighted ID row."""
        try:
            id_tbl = self.query_one("#fs-id-table", DataTable)
            row_key = str(id_tbl.get_row_at(id_tbl.cursor_row)[1].plain).strip()
            # row_key = the actual can_id int stored as row key string
            cursor_key = str(
                list(id_tbl.rows.keys())[id_tbl.cursor_row]
            )
            self._toggle_row_by_key(cursor_key)
        except Exception:
            pass
    ...
    def action_pop_screen(self) -> None:
        ...
        self.app._fullscreen_monitor = None  # type: ignore[attr-defined]
        self.app.pop_screen()
...
class CANBusTUI(App):
    ...
    def action_show_details(self) -> None:
        """Push the Details screen and populate it with current data."""
    ...
    def _update_statistics(self) -> None:
        """Guard against re-entrant calls then delegate to the actual update."""
    ...
    def _trace_export_csv(self) -> None:
        """Export all trace records to a timestamped CSV file in the working directory."""
    ...
    def _do_connect(self) -> None:
        """Open the serial port and initialise the CAN dongle."""
    ...
    def _update_monitor_table(self) -> None:
        ...
        for can_id in self._sorted_ids(rows):
            ...
            if can_id not in self._table_rows:
                table.add_row(
                    id_str, type_str, dlc_str, rate_str, cnt_str,
    ...
    def _calculate_fps(self) -> None:
        """Atomically snapshot and reset the FPS counter (called by timer, UI thread)."""
        with self._counters_lock:
